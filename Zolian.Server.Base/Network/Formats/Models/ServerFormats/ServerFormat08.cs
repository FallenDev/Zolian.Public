    ...
    public class ServerFormat08 : NetworkFormat
    {
        ...
        public override void Serialize(NetworkPacketWriter writer)
        {
            ...
            if ((Flags & 0x20) != 0)
            {
                ...
                writer.Write((byte)Math.Clamp(Aisling.Str, 0, ServerSetup.Config.StatCap));
                writer.Write((byte)Math.Clamp(Aisling.Int, 0, ServerSetup.Config.StatCap));
                writer.Write((byte)Math.Clamp(Aisling.Wis, 0, ServerSetup.Config.StatCap));
                writer.Write((byte)Math.Clamp(Aisling.Con, 0, ServerSetup.Config.StatCap));
                writer.Write((byte)Math.Clamp(Aisling.Dex, 0, ServerSetup.Config.StatCap));
            ...
            if ((Flags & 0x08) != 0)
            {
                ...
                writer.Write(Aisling.ExpLevel >= ServerSetup.Config.PlayerLevelCap
                    ? 0
                    : Aisling.ExpNext);
