...
namespace Darkages.Types
{
    ...
    public class LearningPredicate
    {
        ...
        internal string[] MetaData
            => new[]
            {
                $"{(ExpLevelRequired > 0 ? ExpLevelRequired : 0).ToString()}",
                $"{(_template is SkillTemplate template ? template.Icon : ((SpellTemplate) _template).Icon).ToString()}",
                $"{(StrRequired == 0 ? 3 : StrRequired).ToString()}/{(IntRequired == 0 ? 3 : IntRequired).ToString()}/{(WisRequired == 0 ? 3 : WisRequired).ToString()}/{(DexRequired == 0 ? 3 : DexRequired).ToString()}/{(ConRequired == 0 ? 3 : ConRequired).ToString()}",
                $"{(!string.IsNullOrEmpty(SkillRequired) ? SkillRequired : "0")}/{(SkillLevelRequired > 0 ? SkillLevelRequired : 0).ToString()}",
                $"{(!string.IsNullOrEmpty(SpellRequired) ? SpellRequired : "0")}/{(SpellLevelRequired > 0 ? SpellLevelRequired : 0).ToString()}",
                $"{_template.Description}\n\nItems Required: {(ItemsRequired.Count > 0 ? string.Join(",", ItemsRequired.Select(i => i.AmountRequired + " " + i.Item)) : "None")} \nGold: {(GoldRequired > 0 ? GoldRequired : 0).ToString()}\n{Script()}",
            };
        ...
        {
            if (ServerSetup.Config.DevModeExemptions != null && player.GameMaster &&
                ServerSetup.Config.DevModeExemptions.Contains("learning_predicates"))
                return true;
        ...
        private static string NpcLocation(string npcKey)
        {
            ...
            if (!ServerSetup.GlobalMundaneTemplateCache.ContainsKey(npcKey)) return "\nThe location of this ability is not written on the scroll.\n";

            var npc = ServerSetup.GlobalMundaneTemplateCache[npcKey];

            if (!ServerSetup.GlobalMapCache.ContainsKey(npc.AreaID)) return "\nThe location of this ability is not written on the scroll.\n";

            var map = ServerSetup.GlobalMapCache[npc.AreaID];
            {
                return $"\nFrom: {npc.Name}\nLocation: {map.Name}\n";
            }
        }
        ...
        private int CheckItemPredicates(Aisling player, IDictionary<int, Tuple<bool, object>> result, int n)
        {
            ...
            {
                var msg = new StringBuilder(ServerSetup.Config.ItemNotRequiredMsg);
                ...
                {
                    if (!ServerSetup.GlobalItemTemplateCache.ContainsKey(ir.Item))
                    {
                    ...
                    var item = ServerSetup.GlobalItemTemplateCache[ir.Item];
        ...
        private int CheckSpellandSkillPredicates(Aisling player, Dictionary<int, Tuple<bool, object>> result, int n)
        {
            ...
            {
                var skill = ServerSetup.GlobalSkillTemplateCache[SkillRequired];
            ...
            {
                var spell = ServerSetup.GlobalSpellTemplateCache[SpellRequired];
