...
namespace Darkages.Types
{
    public class Party : ObjectManager
    {
        ...
        private static Party CreateParty(Aisling partyLeader)
        {
            ...
            if (!ServerSetup.GlobalGroupCache.ContainsKey(party.Id))
                ServerSetup.GlobalGroupCache.TryAdd(party.Id, party);
        ...
        {
            if (!ServerSetup.GlobalGroupCache.ContainsKey(group.Id)) return;
            if (!ServerSetup.GlobalGroupCache.TryRemove(group.Id, out _)) return;
        ...
        public static void RemovePartyMember(Aisling playerToRemove)
        {
            ...
            if (!ServerSetup.GlobalGroupCache.ContainsKey(playerToRemove.GroupId)) return;
            var group = ServerSetup.GlobalGroupCache[playerToRemove.GroupId];
