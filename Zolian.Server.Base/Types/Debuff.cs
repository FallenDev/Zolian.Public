...
namespace Darkages.Types
{
    public class Debuff
    {
        ...
        {
            ObtainDebuffName(affected, debuff);
            DebuffSpell.OnApplied(affected, debuff);
        ...
        {
            ObtainDebuffName(affected, debuff);
            DebuffSpell.OnDurationUpdate(affected, debuff);
        ...
        private void ObtainDebuffName(Sprite affected, Debuff debuff)
        {
            ...
            DebuffSpell = debuff.Name switch
            {
                ...
                "Mor Cradh" => new debuff_beagcradh(),
                "Cradh" => new debuff_beagcradh(),
        ...
        protected static async void InsertDebuff(Aisling aisling, Debuff debuff)
        {
            ...
            try
            {
                ...
                var debuffNameReplaced = debuff.Name.Replace("'", "''");
                var playerDeBuffs = "INSERT INTO ZolianPlayers.dbo.PlayersDebuffs (DebuffId, Serial, Name, TimeLeft) VALUES " +
                                    $"('{deBuffId}','{aisling.Serial}','{debuffNameReplaced}','{debuff.TimeLeft}')";
            ...
            catch (SqlException e)
            {
                if (e.Message.Contains("PK__Players"))
                {
                    foreach (var client in ServerSetup.Game.Clients.Values)
                    {
                        ...
                        client.SendMessage(0x03, "Issue saving debuff. Contact GM");
        ...
        protected static async void UpdateDebuff(Aisling aisling)
        {
            ...
            try
            {
                ...
                cmd.CommandText =
                    ...
                    $"FROM ZolianPlayers.dbo.PlayersDebuffs AS P INNER JOIN {table} AS T ON P.[Serial] = T.[Serial] AND P.[Name] = T.[Name]; DROP TABLE {table}; " +
                    "COMMIT;";
                await cmd.ExecuteNonQueryAsync();
                await sConn.CloseAsync();
            }
        ...
        public static async void DeleteDebuff(Aisling aisling, Debuff debuff)
        {
            try
            {
                await using var sConn = new SqlConnection(AislingStorage.ConnectionString);
        ...
        private static async Task<bool> CheckOnDebuffAsync(IGameClient client, string name)
        {
            try
            {
    ...
    public class debuff_reaping : Debuff
    {
        ...
        private static string[] Messages => ServerSetup.Config.ReapMessage.Split(new[] { '|' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public override void OnApplied(Sprite affected, Debuff debuff)
        {
            ...
            if (aisling.CurrentMapId == ServerSetup.Config.DeathMap)
            {
        ...
        public override void OnDurationUpdate(Sprite affected, Debuff debuff)
        {
            if (affected.CurrentMapId == ServerSetup.Config.DeathMap)
            {
            ...
            if (affected is Aisling aisling)
            {
                ...
                if (randCheck <= 50)
                {
                    affected.Client.SendMessage(0x02, $"{Messages[Random.Shared.Next(Count) % Messages.Length]}");
                }
        ...
        public override void OnEnded(Sprite affected, Debuff debuff)
        {
            switch (affected)
            {
                case Aisling aisling when !debuff.Cancelled:
                    ...
                    {
                        aisling.Debuffs.TryRemove(value.Name, out _);
                    }
                    foreach (var (_, value) in aisling.Buffs)
                    {
                        aisling.Buffs.TryRemove(value.Name, out _);
                    }
    ...
    public class debuff_fasspoirad : Debuff
    {
        ...
        public override void OnApplied(Sprite affected, Debuff debuff)
        {
            ...
            else
            {
                ...
                foreach (var near in nearby)
                {
                    near.Client.SendAnimation(1, affected, affected);
                    near.Show(Scope.NearbyAislings, new ServerFormat19(26));
                }
            }
        }
    }
