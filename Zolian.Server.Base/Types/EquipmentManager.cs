...
namespace Darkages.Types
{
    public class EquipmentManager
    {
        ...
        public void Add(int displaySlot, Item item)
        {
            ...
            if (!item.Template.Flags.HasFlag(ItemFlags.Equipable)) return;
        ...
        private static async void AddToAislingDb(ISprite aisling, Item item, int slot)
        {
            try
            {
                ...
                var playerInventory = "INSERT INTO ZolianPlayers.dbo.PlayersEquipped (ItemId, Name, Serial, Slot, Color, Cursed, Durability, Identified, ItemVariance, WeapVariance, ItemQuality, OriginalQuality, Stacks, Enchantable) VALUES " +
                                      $"('{item.ItemId}','{templateNameReplaced}','{aisling.Serial}','{slot}','{color}','{item.Cursed}','{item.Durability}','{item.Identified}','{item.ItemVariance}','{item.WeapVariance}','{quality}','{orgQuality}','{item.Stacks}','{item.Enchantable}')";

                var cmd = new SqlCommand(playerInventory, sConn);
            ...
            catch (SqlException e)
            {
                if (e.Message.Contains("PK__Players"))
                {
                    foreach (var client in ServerSetup.Game.Clients.Values)
                    {
                        ...
                        client.SendMessage(0x03, "Issue equipping gear and saving. Contact GM");
