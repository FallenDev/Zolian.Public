...
namespace Darkages.Object
{
    ...
    public class ObjectManager : IObjectManager
    {
        ...
        {
            if (p != null && p(obj))
                ServerSetup.Game.ObjectFactory.AddGameObject(obj);
            else if (p == null)
                ServerSetup.Game.ObjectFactory.AddGameObject(obj);
        }

        public void DelObject<T>(T obj) where T : Sprite => ServerSetup.Game?.ObjectFactory.RemoveGameObject(obj);

        public void DelObjects<T>(T[] obj) where T : Sprite => ServerSetup.Game?.ObjectFactory.RemoveAllGameObjects(obj);

        public T GetObject<T>(Area map, Predicate<T> p) where T : Sprite => ServerSetup.Game?.ObjectFactory.Query(map, p);
        ...
        public IEnumerable<T> GetObjects<T>(Area map, Predicate<T> p) where T : Sprite => ServerSetup.Game?.ObjectFactory.QueryAll(map, p);
