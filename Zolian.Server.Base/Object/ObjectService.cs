...
namespace Darkages.Object
{
    ...
    {
        private readonly ConcurrentDictionary<int, IDictionary<Type, object>> _spriteCollections = new();

        public ObjectService()
        {
            foreach (var map in ServerSetup.GlobalMapCache.Values)
                _spriteCollections.TryAdd(map.ID, new Dictionary<Type, object>
                {
        ...
        public void AddGameObject<T>(T obj) where T : Sprite
        {
            ...
            if (!_spriteCollections.ContainsKey(obj.CurrentMapId)) return;
            if (!_spriteCollections[obj.CurrentMapId].ContainsKey(typeof(T))) return;
        ...
        public T Query<T>(Area map, Predicate<T> predicate) where T : Sprite
        {
            ...
            if (!_spriteCollections.ContainsKey(map!.ID)) return null;
            var sprite = (SpriteCollection<T>)_spriteCollections[map.ID][typeof(T)];
        ...
        public IEnumerable<T> QueryAll<T>(Area map, Predicate<T> predicate) where T : Sprite
        {
            ...
            if (!_spriteCollections.ContainsKey(map.ID)) return null;
            var sprites = (SpriteCollection<T>)_spriteCollections[map.ID][typeof(T)];
        ...
        {
            if (obj != null && !_spriteCollections.ContainsKey(obj.CurrentMapId)) return;
            if (obj == null) return;
